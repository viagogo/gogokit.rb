...
describe GogoKit::Client::Category do
  ...
  let(:root) do
    ...
    genres_link = Roar::Hypermedia::Hyperlink.new(href: 'https://api.com/g')
    root = GogoKit::Root.new
    root.links = {
      'self' => self_link,
      'viagogo:genres' => genres_link
    }
    root
  end
  ...
  describe '#get_category' do
    ...
    it 'performs a request built from the self link of the root resource' do
      ...
      expected_url = self_link.href + '/categories/55'
      allow(client).to receive(:get_root).and_return(root)
      allow(client).to receive(:request).and_return(body: '{}', status: 200)
      client.get_category 55
    ...
    it 'passes the given options in the request' do
      ...
      allow(client).to receive(:get_root).and_return(root)
      allow(client).to receive(:request).and_return(body: '{}', status: 200)
      client.get_category(-1, expected_options)
    ...
    it 'returns {GogoKit::Category} created from the response' do
      allow(client).to receive(:get_root).and_return(root)
      stub_request(:any, /.*/).to_return(body: fixture('genres.json'))
      genres = client.get_category(-1)
      expect(genres).to be_an_instance_of(GogoKit::Category)
    end
  end
